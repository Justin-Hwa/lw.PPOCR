...
- 导入后切换“扫描识别语言”或“阅读顺序”，会清除整个文档的方向缓存并立即重新校正当前页；其他页在下次访问时重新判断，无需重新导入文件。预览区和全屏弹窗都显示正在判断、已旋转、无需旋转、竖排保留或无法判断的状态；“重新校正方向”可重试当前页。重新校正会清除旧识别结果和标注，保留检索字符串，完成后可再次识别。
